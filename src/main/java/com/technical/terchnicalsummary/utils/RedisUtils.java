...
public class RedisUtils {
    ...
    public String get(String key) {
        return key == null ? null : JSON.toJSONString(redisTemplate.opsForValue().get(key));
    }
...
//        redisTemplate.opsForList().leftPushAll(key,"w","x","y");
     ...
     * @return
     */
    public <T> T getObject(String key, Class<T> clazz, int index) {
        RedisConnection connection = redisTemplate.getConnectionFactory().getConnection();
        connection.select(index);
        try {
     ...
     * @param index 分区
     */
    public void setObject(String key, Object value, int index) {
        RedisConnection connection = redisTemplate.getConnectionFactory().getConnection();
        try {
            connection.select(index);
            connection.set(key.getBytes(StandardCharsets.UTF_8), JSON.toJSONString(value).getBytes(StandardCharsets.UTF_8));
        } finally {
            connection.close();
        }
    }
     ...
     * @return
     */
    public boolean delObject(String key, int index) {
        RedisConnection connection = redisTemplate.getConnectionFactory().getConnection();
        connection.select(index);
        try {
